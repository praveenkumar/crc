...
func checkIfDaemonServiceInstalled() error {
	 ...
	 */
	_, stderr, err := powershell.Execute(" Get-Service", constants.DaemonServiceName)
	if err != nil {
		logging.Debugf("%s service is not installed: %v : %s", constants.DaemonServiceName, err, stderr)
		return err
	}
	return nil
}
...
func fixDaemonServiceInstalled() error {
	...
	psScriptContent := genDaemonServiceInstallScript(
		username,
		password,
		tempDir,
		binPathWithArgs,
	...
	_, _, err = powershell.ExecuteAsAdmin("Installing daemon service for CodeReady Containers", psFilePath)
...
func removeDaemonService() error {
	...
	psScriptContent := genDaemonServiceRemovalScript(
		constants.DaemonServiceName,
	)
	...
	_, _, err = powershell.ExecuteAsAdmin("Uninstalling daemon service", psFilePath)
	// wait for the script to finish executing
	time.Sleep(2 * time.Second)
	if err != nil {
		return err
